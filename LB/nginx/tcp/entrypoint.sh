...
DefaultFile=/etc/nginx/tcp/nginx.conf.default
ConfigFile=/etc/nginx/tcp/nginx.conf
...
if [ "$listen" != "" ]; then
  sed -i -e "/#listen1/c listen ${listen}; #listen1" $ConfigFile
fi
if [ "$listen1" != "" ]; then
  sed -i -e "/#listen1/c listen ${listen1}; #listen1" $ConfigFile
fi
if [ "$listen2" != "" ]; then
  sed -i -e "/#listen2/c listen ${listen2}; #listen2" $ConfigFile
fi
if [ "$listen3" != "" ]; then
  sed -i -e "/#listen3/c listen ${listen3}; #listen3" $ConfigFile
fi
