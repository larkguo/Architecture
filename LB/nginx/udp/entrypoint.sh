...
DefaultFile=/etc/nginx/udp/nginx.conf.default
ConfigFile=/etc/nginx/udp/nginx.conf
...
if [ "$listen" != "" ]; then
  sed -i -e "/#listen1/c listen ${listen} udp; #listen1" $ConfigFile
fi
if [ "$listen1" != "" ]; then
  sed -i -e "/#listen1/c listen ${listen1} udp; #listen1" $ConfigFile
fi
if [ "$listen2" != "" ]; then
  sed -i -e "/#listen2/c listen ${listen2} udp; #listen2" $ConfigFile
fi
if [ "$listen3" != "" ]; then
  sed -i -e "/#listen3/c listen ${listen3} udp; #listen3" $ConfigFile
fi
